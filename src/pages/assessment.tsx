...
function WaterGlass({ index, selectedCount, onTap }: { index: number; selectedCount: number; onTap: () => void }) {
  ...
  return (
    <motion.button whileTap={{ scale: 0.9 }} onClick={onTap} className="flex flex-col items-center gap-1">
      <div className={cn('w-9 h-12 rounded-b-xl rounded-t-sm border-2 relative overflow-hidden transition-all duration-300', isActive ? 'border-blue-400' : 'border-gray-200 bg-gray-50')}>
        <motion.div
          className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-blue-400 to-sky-300"
          animate={{ height: isActive ? '85%' : '0%' }}
...
export default function Assessment() {
  ...
  // For single-select that auto-advances
  ...
  // Steps that need explicit Next button (not auto-advance)
    ...
    <Page className="p-6 pt-12 pb-6 bg-white">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* Q4: Hunger Patterns — Coloured Left-border Cards */}
          {step === 3 && (
